...
class Department {
    ...
    getUnBusyDeveloper() {
        const array = this.getUnBusyDevelopers().sort((developer1, developer2) => developer2.skill - developer1.skill);
        return array.pop();
    }
    /**
    ...
    workEnd() {
        ...
        this.projects
            .filter(project => project.progress >= project.difficulty) /* Массив проектов, над которыми завершена работа */
            .forEach(project => {
            project.progress = 0;
            project.difficulty = 1;
            const developers = this.developers.filter(developer => developer.project === project);
